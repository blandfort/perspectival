...
@pytest.mark.parametrize(
    "item_log_likelihoods1, item_log_likelihoods2, min_value, max_value",
    [
        ((-100, -0.01), (-0.01, -100), 0.97, 1),  # Perfect disagreement
        ((-100, -110), (-110, -100), 0.05, 0.2),
        ((-1, -10), (-3, -4), 0, 0),  # Same choice
        # Additional irrelevant options shouldn't have an effect
        ((-100, -0.01, -500), (-0.01, -100, -500), 0.97, 1),
        ((-500, -100, -1000, -110), (-500, -110, -1000, -100), 0.05, 0.2),
    ],
)
def test_disagreement_score(
    item_log_likelihoods1, item_log_likelihoods2, min_value, max_value
):
    model1 = "dummy1"
    model2 = "dummy2"
    ...
    disagreements = LogDisagreement.compute(
